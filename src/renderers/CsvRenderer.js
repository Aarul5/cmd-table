...
var CsvRenderer = /** @class */ (function () {
    ...
    CsvRenderer.prototype.render = function (table) {
        var _this = this;
        var visibleIndices = table.columns
        ...
        rows.push.apply(rows, table.rows.map(function (row) {
            return _this.drawRow(visibleIndices.map(function (item) { var _a, _b; return (_b = (_a = row.cells[item.index]) === null || _a === void 0 ? void 0 : _a.getString()) !== null && _b !== void 0 ? _b : ''; }));
        }));
        return rows.join('\n');
    };
    ...
    CsvRenderer.prototype.escape = function (value) {
        if (value.includes(this.delimiter) || value.includes(this.quote) || value.includes('\n')) {
            return (this.quote +
                value.replace(new RegExp(this.quote, 'g'), this.quote + this.quote) +
                this.quote);
        }
        return value;
    };
